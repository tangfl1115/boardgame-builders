...
export const useGameData = () => {
    ...
    const [gameData, setGameData] = useState(() => {
        const saved = localStorage.getItem('loveLetterGameData_v2');
        if (saved) {
            try {
    ...
    useEffect(() => {
        localStorage.setItem('loveLetterGameData_v2', JSON.stringify(gameData));
    }, [gameData]);
    ...
    const exportData = () => {
        ...
        downloadAnchorNode.setAttribute("download", "love_letter_backup.json");
