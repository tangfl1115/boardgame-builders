...
export const Card = ({ card, size = 'standard', inkSaver = false, version }) => {
    ...
    return (
        ...
        >
            ...
            >
                {/* Number Badge in Clay Style */}
                <div 
                    className={`
                        absolute flex items-center justify-center font-sans font-black z-20 force-print-bg
                        ${isLargePrint ? 'w-28 h-28 text-7xl -left-2 top-2' : (isMediumPrint ? 'w-20 h-20 text-5xl left-0 top-2' : 'w-14 h-14 text-4xl left-0 -top-1')}
                    `}
                    style={{
                ...
                {/* Title */}
                <div className={`w-full flex justify-center items-center ${isLargePrint ? 'pl-24' : (isMediumPrint ? 'pl-16' : 'pl-12')}`}>
                ...
                {card.imagePath && (
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ paddingBottom: isLargePrint ? '80px' : '40px' }}>
                        <div style={{
                            ...
                            {/* HUGE NUMBER ON TOP */}
                            <div className="absolute font-black text-white z-10 drop-shadow-2xl" style={{
                                fontSize: isLargePrint ? '110px' : (isMediumPrint ? '70px' : '52px'),
                                textShadow: '3px 3px 0 #1f2937, -1px -1px 0 #1f2937, 1px -1px 0 #1f2937, -1px 1px 0 #1f2937'
                            }}>
                                {card.value}
                            </div>
                        </div>
