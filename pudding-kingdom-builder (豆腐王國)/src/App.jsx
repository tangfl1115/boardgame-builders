...
function App() {
    ...
    const [currentVersion, setCurrentVersion] = useState('pudding-kingdom');
                ...
                <div className="max-w-7xl mx-auto p-2 md:p-4 print-hidden">
                    <header className="bg-white p-3 rounded-xl shadow border border-stone-300 flex flex-col gap-3">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                            <div>
                                <h1 className="text-xl md:text-2xl font-black text-[#d97706] flex items-center gap-2">
                                    <Icon path={icons.Crown} color="#d97706" />
                                    布丁王國生成器
                                    <span className="text-xs px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full font-bold ml-2">支援離線編輯</span>
                                ...
                                <div className="flex gap-2 mt-1">
                                    {Object.values(gameData).map(v => (
                                        <button 
                                            ...
                                            className={`px-2 py-0.5 rounded text-xs font-bold border transition-colors ${currentVersion === v.id ? 'bg-[#d97706] text-white border-[#d97706]' : 'bg-white text-stone-500 border-stone-300'}`}
                                        >
                                            {v.name}
                                        </button>
                                    ))}
                        ...
                        <div className="flex flex-wrap gap-2 border-t pt-2 border-stone-100 justify-center">
                            <button onClick={() => setView('gallery')} className={`px-3 py-1 rounded text-xs font-bold ${view === 'gallery' ? 'bg-[#d97706] text-white' : 'text-stone-500 hover:bg-stone-100'}`}>瀏覽</button>
